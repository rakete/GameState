...
data ObjMesh a b = ObjMesh
    { objmesh_name :: String
    , objmesh_data :: ([GL.Vertex3 a],[GL.Normal3 a],[GL.TexCoord2 a],[Indices b])
    , groups :: IM.IntMap (ObjVertexGroup b)
    }
    deriving Show

data ObjScene a b = ObjScene
    { mtllib :: Maybe (M.Map String (ObjMaterial a))
    , objects :: IM.IntMap (ObjMesh a b)
    }
...
parseVertices :: (Fractional c) => GenParser Char ParserState ([GL.Vertex3 c], [GL.Normal3 c], [GL.TexCoord2 c])
parseVertices = do
    (vertex_list,normal_list,texcoord_list) <- permute $
        ...
        <|?> ([],many1 $ do
            ...
            ny <- parseComponent
            nz <- parseComponent
            return $ GL.Normal3 nx ny nz)
        <|?> ([],many1 $ do
            reserved "vt"
            u <- parseComponent
            v <- parseComponent
            return $ GL.TexCoord2 u v)
...
parseVertexGroup = do
    ...
    indices <- many1 $ do -- [([Integral a],[Integral a],[Integral a]),...]
        ...
        ixs <- many3 $ try (do
                v <- do
                    v' <- natural
                    return $ fromInteger (v'-1-v_offset)
                symbol "/"
                t <- option Nothing $ do
                    t' <- natural
                    return $ Just $ fromInteger (t'-1-t_offset)
                symbol "/"
                n <- option Nothing $ do
                    n' <- natural
                    return $ Just $ fromInteger (n'-1-n_offset)
                return (v,n,t))
            <|> (do
                   v' <- natural
                   return $ (fromInteger (v'-1-v_offset),Nothing,Nothing))
        return ixs
...
parseObject :: (GL.VertexComponent c, RealFloat c, Enum c, Integral i) => String -> GenParser Char ParserState (ObjMesh c i)
parseObject fallbackname = do
    ...
    -- correct offsets for every group, also triangulate the indices WHICH REALLY SUCKS AND I SHOULD NOT DO IT!
    let (groups,fitted_indices,_) = foldl (\(groups',indices',offset') (typ,name,material,iss) ->
                                        let iss' = concat $ map (snd . triangulate verticesMap) $ fitIndices indicesMap iss []
            ...
            case M.lookup (vi,ni,ti) m of
                ...
                Nothing ->
                    let newIndex = fromIntegral $ length vs
                        vs' = (vsm IM.! vi) : vs
                        ns' = if isJust ni
                               then (IM.findWithDefault (GL.Normal3 1.0 0.0 0.0) (fromJust ni) nsm) : ns
                               else (GL.Normal3 0.0 1.0 0.0) : ns
                        ts' = if isJust ti
                               then (IM.findWithDefault (GL.TexCoord2 0.0 0.0) (fromJust ti) tsm) : ts
                               else (GL.TexCoord2 0.0 0.0) : ts
                        is' = newIndex : is
...
parseObjScene :: (GL.VertexComponent c, Fractional c, RealFloat c, Enum c, Integral i) => String -> GenParser Char ParserState (Maybe FilePath,ObjScene c i)
...
parseObjFile :: (GL.ColorComponent c,GL.VertexComponent c, Fractional c, RealFloat c, Enum c, Integral i) => FilePath -> IO (Either ParseError (ObjScene c i))
...
parseObjMaterial :: (GL.ColorComponent c, Fractional c) => GenParser Char ParserState (ObjMaterial c)
parseObjMaterial = do
    reserved "newmtl"
    name <- identifier
    permute $
        ...
        <|?> (Nothing,do -- d
            ...
            cg <- parseComponent
            cb <- parseComponent
            return $ Just $ GL.Color4 cr cg cb 1.0)
        <|?> (Nothing,do -- e
            reserved "Ke"
            cr <- parseComponent
            cg <- parseComponent
...
parseObjMaterialLib :: (GL.ColorComponent c, Fractional c) => GenParser Char ParserState (M.Map String (ObjMaterial c))
...
parseMtlFile :: (GL.ColorComponent c, Fractional c) => FilePath -> IO (Either ParseError (M.Map String (ObjMaterial c)))
